...
double get_expertise(const Special_stats& special_stats, Weapon_type weapon_type, Weapon_socket weapon_socket)
{
    double expertise;
    switch (weapon_type)
    {
    case Weapon_type::sword:
        expertise = special_stats.sword_expertise + special_stats.expertise;
        return expertise;
    case Weapon_type::axe:
        expertise = special_stats.axe_expertise + special_stats.expertise;
        return expertise;
    case Weapon_type::mace:
        expertise = special_stats.mace_expertise + special_stats.expertise;
        return expertise;
    default:
        return special_stats.expertise;
    }
}
                                   ...
                                   double sim_time, const std::vector<Use_effect>& use_effects)
{
    ...
    double mh_expertise = get_expertise(special_stats, mh_wep.type, mh_wep.weapon_socket);
    double mh_hit_crit_expertise_ap = get_hit_crit_expertise_ap_equivalent(special_stats, mh_expertise);

    double oh_expertise = get_expertise(special_stats, oh_wep.type, mh_wep.weapon_socket);
    double oh_hit_crit_expertise_ap = get_hit_crit_expertise_ap_equivalent(special_stats, oh_expertise);
                                   ...
                                   const std::vector<Use_effect>& use_effects)
{
    double attack_power = special_stats.attack_power;
    double mh_expertise = get_expertise(special_stats, mh_wep.type, mh_wep.weapon_socket);
    double hit_crit_expertise_ap = get_hit_crit_expertise_ap_equivalent(special_stats, mh_expertise);
...
double get_hit_crit_expertise_ap_equivalent(const Special_stats& special_stats)
{
    ...
    // Dodge chance
    double dodge_chance = std::max(5 + skill_diff * 0.1, 5.0) - special_stats.expertise + special_stats.mace_expertise + special_stats.axe_expertise;
...
double estimate_special_stats_high(const Special_stats& special_stats)
{
    ...
    high_estimation +=
        special_stats.attack_power + special_stats.hit * hit_w + special_stats.critical_strike * crit_w + ap_from_expertise;
    return high_estimation;
}

double estimate_special_stats_low(const Special_stats& special_stats)
{
    double max_expertise = std::max(special_stats.axe_expertise, 0.0);
    max_expertise = std::max(special_stats.sword_expertise, max_expertise);
    max_expertise = std::max(special_stats.mace_expertise, max_expertise);
    ...
    low_estimation += special_stats.attack_power + special_stats.hit * hit_w_cap +
                      special_stats.critical_strike * crit_w_cap + max_expertise * expertise_w_cap;
